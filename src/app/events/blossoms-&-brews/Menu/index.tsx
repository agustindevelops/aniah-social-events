  ...
  return (
    ...
    >
      <div className='text-center'>
        <h2 className='font-playfair mb-4 text-6xl font-bold'>Le Menu</h2>
      </div>
      {data.map(({ title, items }) => (
        <div key={title} className='my-8'>
          <h3 className='font-playfair decoration-peach-400 mb-4 text-4xl font-bold underline decoration-wavy'>
            {title}
          </h3>
          <ul className=''>
            {items.map((item) => {
